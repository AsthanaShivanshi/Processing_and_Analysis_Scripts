...
def compute_metrics(obs, pred):
    ...
    for t in range(obs.shape[0]):
        ...
        if not np.any(mask):
            continue  # Skip this timestep if all values are NaN
        hr_img = hr_img[mask]
...
ddim_median_temp = xr.open_dataset("../../Downscaling_Models/DDIM_conditional_derived/outputs/ensmean_ddim_eta0_four_samples.nc")["temp"].sel(time=slice("2011-01-01","2023-12-31")) #Median calculted with cdo ensmedian, saved in the outputs folder 
...
ddim_median_precip = xr.open_dataset("../../Downscaling_Models/DDIM_conditional_derived/outputs/ensmean_ddim_eta0_four_samples.nc")["precip"].sel(time=slice("2011-01-01","2023-12-31")) #Mean calculated with cdo ensmean, saved in the outputs folder 
...
temp_metrics = {
    ...
    'DDIM four sample ensmean': compute_metrics(obs_temp, ddim_median_temp_hr)
}
...
precip_metrics = {
    ...
    'DDIM four sample ensmean': compute_metrics(obs_precip, ddim_median_precip_masked)
}
