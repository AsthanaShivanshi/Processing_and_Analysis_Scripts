...
#Tackling missing coords issue in some inference files. 
...
def gridwise_temporal_lsd(obs, pred, n_fft=256, eps=1e-8, n_jobs=-1, mask=None):
    ...
    for i in range(N):
        for j in range(E):
            if mask is not None and not bool(mask.values[i, j]):
                continue
            tasks.append((i, j))
...
def ensemble_rmse(obs, ens_pred):
    rmse_list = []
    for i in range(ens_pred.sizes["sample"]):
        rmse_val = rmse(obs, ens_pred.isel(sample=i))
...
def pitd_for_grid(i, j, obs_arr, ens_arr, bins):
    ...
    # PIT_t = F_forecast_t(observation_t)
    #
    # Tie-corrected empirical PIT:
    # useful especially for precip, where many values may be exactly 0.
...
def gridwise_temporal_pitd(obs, ens_pred, bins=20, n_jobs=-1, mask=None):
    ...
    for i in range(N):
        for j in range(E):
            if mask is not None and not np.isfinite(mask.values[i, j]):
                continue
            if mask is not None and not bool(mask.values[i, j]):
                continue
            tasks.append((i, j))
...
test_precip_bilinear=(xr.open_dataset("../Downscaling_Models/Dataset_Setup_I_Chronological_12km/RhiresD_step3_bilinear.nc")["RhiresD"].sel(time=slice("2015-01-01", "2023-12-31"))).where(Swiss_Mask_HR)
...
#Unet
test_temp_unet=(xr.open_dataset("../Downscaling_Models/output_inference/unet_downscaled_test_set_2015_2023.nc")["temp"].sel(time=slice("2015-01-01", "2023-12-31"))).where(Swiss_Mask_HR)
test_precip_unet=(xr.open_dataset("../Downscaling_Models/output_inference/unet_downscaled_test_set_2015_2023.nc")["precip"].sel(time=slice("2015-01-01", "2023-12-31"))).where(Swiss_Mask_HR)
...
test_temp_ddim=(xr.open_dataset("../Downscaling_Models/output_inference/ddim_downscaled_test_set_S30_samples10_eta0.0.nc")["temp"].sel(time=slice("2015-01-01", "2023-12-31"))).where(Swiss_Mask_HR)
test_precip_ddim=(xr.open_dataset("../Downscaling_Models/output_inference/ddim_downscaled_test_set_S30_samples10_eta0.0.nc")["precip"].sel(time=slice("2015-01-01", "2023-12-31"))).where(Swiss_Mask_HR)
...
test_temp_ddim_median=(xr.open_dataset("../Downscaling_Models/output_inference/ddim_downscaled_test_set_S30_samples10_eta0.0_median.nc")["temp"].sel(time=slice("2015-01-01", "2023-12-31"))).where(Swiss_Mask_HR)
test_precip_ddim_median=(xr.open_dataset("../Downscaling_Models/output_inference/ddim_downscaled_test_set_S30_samples10_eta0.0_median.nc")["precip"].sel(time=slice("2015-01-01", "2023-12-31"))).where(Swiss_Mask_HR)
...
for name, pred in tqdm_auto(models_temp.items(), desc="Processing temp"):
    ...
    mae_val = mae(spatial_mean_ts(pred_med), spatial_mean_ts(test_temp))
    psnr_val = psnr(spatial_mean_ts(pred_med), spatial_mean_ts(test_temp))

    metrics[f"{name}_temp"] = {
        "CRPS_temp": np.nanmean(crps_grid),
        "LSD_temp_ensmedian": np.nanmean(lsd_grid),
        "SSIM_temp_ensmedian": ssim_val,
        "RMSE_temp_ensmedian": rmse_val,
        "MAE_temp_ensmedian": mae_val,
        "PSNR_temp_ensmedian": psnr_val,
        "PITD_temp": np.nanmean(pitd_grid),
    }
...
for name, pred in tqdm_auto(models_precip.items(), desc="Processing precip"):
    ...
    mae_val = mae(spatial_mean_ts(pred_med), spatial_mean_ts(test_precip))
    psnr_val = psnr(spatial_mean_ts(pred_med), spatial_mean_ts(test_precip))




    metrics[f"{name}_precip"] = {
        "CRPS_precip": np.nanmean(crps_grid),
        "LSD_precip_ensmedian": np.nanmean(lsd_grid),
        "SSIM_precip_ensmedian": ssim_val,
        "RMSE_precip_ensmedian": rmse_val,
        "MAE_precip_ensmedian": mae_val,
        "PSNR_precip_ensmedian": psnr_val,
        "PITD_precip": np.nanmean(pitd_grid),
    }
...
metric_df.to_csv("Analysis/Paper_Stats/SR_metrics_cobweb.csv")
