...
# inability of the model to predict dry days (exact zeroes, very small values around zero instead. )

WET_THRESHOLD = 0.05  # mm/day
...
def zero_inflated_pit(obs, ref_values, threshold=WET_THRESHOLD):
    ...
    zero_mask = (obs_thresholded == 0.0)

    # Show the point mass at zero explicitly
    pit[zero_mask] = 0.0
...
ref_pit, p_zero_ref = zero_inflated_pit(ref_spatial_mean, ref_spatial_mean)
...
for eta, ds_path in zip(etas, downscaled_paths):
    ...
    for s in range(temp_spatial_mean.shape[1]):
        pit_s, _ = zero_inflated_pit(temp_spatial_mean[:, s], ref_spatial_mean)
    ...
    ax.bar(
        ...
        pit_hist,
        width=width,
        label=f"DDIM pooled samples",
        color=VARIABLE_COLORS["temp"],
        alpha=0.85,
        edgecolor='black'
    )
