...
for city, (lat, lon) in cities.items():
    ...
    plt.plot(obs_q, ddim_median_q, 'o', label='DDIM Median', alpha=0.4, color='red')
    plt.plot([obs_q.min(), obs_q.max()], [obs_q.min(), obs_q.max()], 'k--', label='1:1 Line')
    plt.xlabel(f'Observed Quantiles ({city}) for temperature')
    plt.ylabel(f'Model Quantiles ({city}) for temperature')
