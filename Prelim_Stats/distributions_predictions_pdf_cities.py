...
def get_lat_lon(da):
    if 'lat' in da.dims and 'lon' in da.dims:
        ...
        # Note: order is E, N, so swap for lat/lon
        return da['N'].values, da['E'].values, 'N', 'E'
    else:
...
def plot_city_pdf(city_coords, obs, unet, recon, varname, city_name="City"):
    ...
    # Finding closest pixel
    obs_lon2d, obs_lat2d = np.meshgrid(obs_lon, obs_lat)
    dist = np.sqrt((obs_lat2d - city_coords[0])**2 + (obs_lon2d - city_coords[1])**2)
    ...
    unet_lon2d, unet_lat2d = np.meshgrid(unet_lon, unet_lat)
    dist_unet = np.sqrt((unet_lat2d - city_coords[0])**2 + (unet_lon2d - city_coords[1])**2)
    lat_idx_unet, lon_idx_unet = np.unravel_index(np.argmin(dist_unet), dist_unet.shape)
    unet_series = unet.isel({unet_lat_dim: lat_idx_unet, unet_lon_dim: lon_idx_unet}).values.flatten()


    recon_lon2d, recon_lat2d = np.meshgrid(recon_lon, recon_lat)
    dist_recon = np.sqrt((recon_lat2d - city_coords[0])**2 + (recon_lon2d - city_coords[1])**2)
    ...
    plt.hist(obs_series, bins=50, density=True, alpha=0.5, label="Observations")
    plt.hist(unet_series, bins=50, density=True, alpha=0.5, label="UNet predictions from 1971 time series")
    plt.hist(recon_series, bins=50, density=True, alpha=0.5, label="Reconstructed Time Series")
    plt.title(f"{varname} PDF at {city_name} ({city_coords[0]:.4f}, {city_coords[1]:.4f})")
    plt.xlabel(varname)
    plt.ylabel("Density")
    plt.legend()
    plt.tight_layout()
    plt.savefig(f"../Outputs/pdf_{varname}_{city_name}.png")
    plt.close()
...
if __name__ == "__main__":
    ...
    obs_ds = xr.open_dataset(f"../../data_1971_2023/HR_files_full/{obs_var}_1971_2023.nc", chunks={"time": 100}).sel(time=slice("2011-01-01", "2020-12-31"))
